...
async function checkDartServerConnection() {
  try {
    const response = await axios.get('http://localhost:4242/status', { timeout: 5000 });
    const statusMsg = '✅ Eden\'s brain server is responding';
    console.log(statusMsg);
    logToFile(statusMsg);
  } catch (err) {
    const errorMsg = '❌ Cannot connect to Eden\'s brain server at localhost:4242. Please start prompt_server.dart separately.';
